...
const forecast = (latitude, longitude,  callback) => {
    const url = 'http://api.weatherstack.com/current?access_key=b004106eeade0f405ef5f607837abc92&query=' + latitude + ',' + longitude + '&units=m'
    postman_request({url, json: true}, (error, {body} = {}) => {
        if(error){
            ...
            callback(undefined, body.current.weather_descriptions[0] + ' It is currently ' + body.current.temperature + ' degree out.' +
                   ' It feals like ' + body.current.feelslike +' degree out. The humidity is ' + body.current.humidity + '% .')
        }
    })
